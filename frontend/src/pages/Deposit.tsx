...
import { MARKETS, USDC_MINT, STRIKES_USD, EXPIRY_DAYS } from '../lib/constants';
import { api } from '../lib/api';

const MINT_FEE_BPS = 10; // 0.10 %

// Labels shown on expiry buttons
...
export function Deposit({ onNavigate }: Props) {
  ...
  const [strikeUsd, setStrikeUsd] = useState(STRIKES_USD[6]); // default $180
  const [expiryDays, setExpiryDays] = useState(EXPIRY_DAYS[0]);
  const [side, setSide] = useState<'CALL' | 'PUT'>('CALL');   // LONG vault = CALL
  ...
  useEffect(() => {
    if (wallet.connected && wallet.publicKey) fetchBalance();
  ...
  const handleMint = useCallback(async () => {
    ...
    try {
      ...
      const { tx: vaultTx, longMint, shortMint } = await buildCreateRootVaultTx(
        ...
        collateralMicro, strikeMicro, expiryTs, vaultSideNum
      );
  ...
  }, [wallet, connection, amountNum, market, strikeUsd, expiryDays, side, vaultSideNum]);
    ...
    return (
      <div className="min-h-screen bg-void pt-20 flex items-start justify-center px-4">
        <div className="mt-16 w-full max-w-lg border border-accent/40 bg-surface p-8">
          ...
          {/* Summary */}
          <div className="grid grid-cols-3 gap-2 mb-5 font-mono text-xs">
            <div className="border border-wire p-3">
              <div className="text-[10px] uppercase tracking-widest text-fg-muted mb-1">Type</div>
              <div className={result.side === 'CALL' ? 'text-bull' : 'text-bear'}>{result.side}</div>
            </div>
            <div className="border border-wire p-3">
              <div className="text-[10px] uppercase tracking-widest text-fg-muted mb-1">Strike</div>
              <div className="text-fg">${result.strikeUsd}</div>
            </div>
            <div className="border border-wire p-3">
              <div className="text-[10px] uppercase tracking-widest text-fg-muted mb-1">Expiry</div>
              <div className="text-fg">{result.expiryDays}d</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <div className="border border-wire p-4">
              <div className="font-mono text-[10px] uppercase tracking-widest text-fg-muted mb-1">
                {result.side === 'CALL' ? 'CALL' : 'CAP'} Token
              </div>
              <div className="font-mono text-xs text-fg truncate" title={result.longMint}>
                {result.longMint.slice(0, 16)}…
              </div>
            </div>
            <div className="border border-wire p-4">
              <div className="font-mono text-[10px] uppercase tracking-widest text-fg-muted mb-1">
                {result.side === 'CALL' ? 'FLOOR' : 'PUT'} Token
              </div>
              <div className="font-mono text-xs text-fg truncate" title={result.shortMint}>
                {result.shortMint.slice(0, 16)}…
              </div>
            </div>
          </div>

          {/* Next steps */}
          <div className="mb-6 border border-accent/25 bg-accent/5 px-4 py-3 text-xs font-mono text-fg-muted leading-relaxed space-y-1">
            <p className="text-fg font-semibold tracking-wide mb-1">What you can do now</p>
            <p>
              <span className="text-accent">→ Sell for premium:</span>{' '}
              List the {result.side === 'CALL' ? 'FLOOR' : 'CAP'} token and keep the pure {result.side}.
            </p>
            <p>
              <span className="text-accent">→ Deepen the chain:</span>{' '}
              Split your {result.side === 'CALL' ? 'CALL' : 'PUT'} into a higher-strike position — no extra collateral.
            </p>
            <p>
              <span className="text-accent">→ Spread strategy:</span>{' '}
              Hold both tokens for a bounded payoff at settlement.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              TRADE {result.side === 'CALL' ? 'CALL' : 'CAP'} TOKEN
              <ArrowRight size={14} />
            </button>
            <button
              onClick={() => onNavigate(`#/app/split/${result.longMint}`)}
        ...
        <div className="border border-accent/30 bg-surface p-6 space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              <button
                ...
                className="w-full flex items-center justify-between px-4 py-3 border border-wire bg-surface-2 text-fg font-mono text-sm hover:border-accent/50 transition-colors focus-visible:outline-2 focus-visible:outline-accent"
              >
                {market.label}
          ...
          <div>
            ...
            <div className="mt-1.5 font-mono text-[10px] text-fg-muted">
              {side === 'CALL'
                ? 'CALL profits when price is above strike · FLOOR is the bounded complement'
                : 'PUT profits when price is below strike · CAP is the bounded complement'}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-7 gap-1.5">
              {STRIKES_USD.map(s => (
                <button key={s} type="button"
                  onClick={() => setStrikeUsd(s)}
                  className={`py-2 font-mono text-[10px] tracking-wide transition-colors border ${
                    strikeUsd === s
                      ? 'bg-accent/15 border-accent text-accent'
                      : 'border-wire text-fg-muted hover:border-accent/40 hover:text-fg'
                  }`}
                >
                  ${s}
                </button>
          ...
          <div>
            ...
            <div className="relative">
              <input
                id="deposit-amount" type="number" min="0" step="0.01"
              ...
              />
              <span className="absolute right-4 top-1/2 -translate-y-1/2 font-mono text-xs text-fg-muted">USDC</span>
            </div>
          </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-mono text-xs text-fg mb-1">
                    {usdcBalance === 0 ? 'No test USDC in your wallet' : `Need ${amountNum.toFixed(2)} USDC, have ${usdcBalance?.toFixed(2)}`}
                  </div>
                  <div className="font-mono text-[10px] text-fg-muted mb-3">
                    Get 1,000 devnet USDC instantly.
                  </div>
                  ...
                  >
                    <Droplets size={12} /> GET 1,000 TEST USDC
                  </button>
                </div>
              </div>
            </div>
          ...
          {fauceting && (
            <div className="flex items-center gap-2 border border-accent/20 bg-accent/5 p-3">
              ...
              <span className="font-mono text-xs text-fg-muted">Minting test USDC…</span>
            </div>
          )}
