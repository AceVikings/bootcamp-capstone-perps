...
function epochPDAs(
  ...
  return { epochPda, longMint, shortMint, longLongMint, longShortMint, shortLongMint, shortShortMint };
}
...
describe("TPP Protocol", () => {
  ...
  const program = new anchor.Program(IDL as anchor.Idl, provider) as unknown as Program<TppProtocol>;
  ...
  let longLongMint: PublicKey;
  let longShortMint: PublicKey;
  let shortLongMint: PublicKey;
  let shortShortMint: PublicKey;
  ...
  // ── Before: global setup ──────────────────────────────────────────────────
  before(async () => {
    // Fund admin wallet on localnet (anchor test starts a fresh validator)
    ...
    longLongMint = pdas.longLongMint;
    longShortMint = pdas.longShortMint;
    shortLongMint = pdas.shortLongMint;
    shortShortMint = pdas.shortShortMint;
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // 4-5. Merged redemption (burn equal LONG + SHORT → get 1:1 collateral)
  // ──────────────────────────────────────────────────────────────────────────

  it("4-5. redeems merged LONG + SHORT tokens", async () => {
    await setOraclePrice(oracleKp, PRICE_100);
    ...
    // Redeem 1/4 of both sides (must be equal amounts)
    const redeemAmt = new BN(longBefore.amount.toString()).divn(4);

    await program.methods
      .redeemMerged(EPOCH_ID, VAULT_0, redeemAmt)
      .accounts({
        epoch: epochPda,
        vault: adminVaultPda,
        callerLongAta: adminLongAta,
        callerShortAta: adminShortAta,
        longMint,
        shortMint,
        callerCollateral: adminUsdcAta,
        vaultCollateral: ata(usdcMint, adminVaultPda),
        treasuryCollateral: ata(usdcMint, feeTreasury),
        collateralMint: usdcMint,
        feeTreasury,
    ...
    // Both token balances decreased by redeemAmt
    assert.equal(
    ...
    // USDC balance increased (got payout minus fee)
  ...
  it("9. pause blocks minting but NOT redemption", async () => {
    ...
    if (longBal.amount > 0n) {
      ...
      await program.methods
        .redeemMerged(EPOCH_ID, VAULT_0, new BN(1))
        .accounts({
          epoch: epochPda,
          vault: adminVaultPda,
          callerLongAta: adminLongAta,
          callerShortAta: ata(shortMint, admin.publicKey),
          longMint,
          shortMint,
          callerCollateral: ata(usdcMint, admin.publicKey),
          vaultCollateral: ata(usdcMint, adminVaultPda),
          treasuryCollateral: ata(usdcMint, feeTreasury),
          collateralMint: usdcMint,
          feeTreasury,
  ...
  it("12. rejects stale oracle price (age > max_oracle_age_secs = 60 s)", async () => {
    ...
    const { epochPda: ep99, longMint: lm99, shortMint: sm99, longLongMint: llm99, longShortMint: lsm99, shortLongMint: slm99, shortShortMint: ssm99 } = epochPDAs(
      program,
      assetKey.publicKey,
      epochId99
    );
  ...
  it("20. partial redemption of both long and short at entry price is symmetric", async () => {
    ...
    await program.methods
      .redeemMerged(EPOCH_ID, VAULT_2, redeemAmt)
      .accounts({
        epoch: epochPda,
        vault: vault2,
        callerLongAta: adminLongAta,
    ...
    // Verify payout ≈ redeemAmt (1:1 minus redeem fee)
    const expectedGross = redeemAmt.toNumber();
  ...
  it("21. creates independent epoch 1 with its own mint pair and price band", async () => {
    ...
    const { epochPda: epoch1Pda, longMint: lm1, shortMint: sm1, longLongMint: llm1, longShortMint: lsm1, shortLongMint: slm1, shortShortMint: ssm1 } = epochPDAs(
  ...
  it("24. zero-fee redemption returns gross payout with no deduction", async () => {
    ...
    // Redeem 50% of tokens – expect gross = net (no redeem fee)
    const user5LongAta = ata(longMint, user5.publicKey);
    const user5ShortAta = ata(shortMint, user5.publicKey);
    ...
    await program.methods
      .redeemMerged(EPOCH_ID, new BN(0), redeemAmt)
      .accounts({
        epoch: epochPda, vault: user5Vault,
        callerLongAta: user5LongAta,
        callerShortAta: user5ShortAta,
        longMint, shortMint,
        callerCollateral: user5UsdcAta,
        vaultCollateral: ata(usdcMint, user5Vault),
        treasuryCollateral: ata(usdcMint, feeTreasury),
        collateralMint: usdcMint, feeTreasury,
        config: protocolConfig,
        caller: user5.publicKey,
        tokenProgram: TOKEN_PROGRAM_ID, associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID,
        systemProgram: SystemProgram.programId,
      })
      .signers([user5])
    ...
    // gross = redeemAmt; fee = 0; net = redeemAmt
  ...
  it("25. rejects redeem when amount exceeds token balance", async () => {
    ...
    try {
      await program.methods
        .redeemMerged(EPOCH_ID, VAULT_0, tooMany)
        .accounts({
          epoch: epochPda,
          vault: adminVaultPda,
          callerLongAta: adminLongAta,
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // 26. Redeeming with wrong token mint fails
  // ──────────────────────────────────────────────────────────────────────────

  it("26. rejects redeem when mints do not match epoch", async () => {
    await setOraclePrice(oracleKp, PRICE_100);
    ...
    try {
      // Pass shortMint as longMint (wrong mint)
      await program.methods
        .redeemMerged(EPOCH_ID, VAULT_0, new BN(1))
        .accounts({
          epoch: epochPda,
          vault: adminVaultPda,
          callerLongAta: adminShortAta, // wrong ATA
          callerShortAta: adminShortAta,
          longMint: shortMint, // wrong mint
          shortMint,
          callerCollateral: ata(usdcMint, admin.publicKey),
          vaultCollateral: ata(usdcMint, adminVaultPda),
          treasuryCollateral: ata(usdcMint, feeTreasury),
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // Stage 2: split_claim and merge_claims tests
  // ──────────────────────────────────────────────────────────────────────────

  it("29. epoch stores depth-2 mint addresses after createEpoch", async () => {
    const epoch = await program.account.epoch.fetch(epochPda);
    assert.ok(epoch.longLongMint.equals(longLongMint), "longLongMint stored");
    assert.ok(epoch.longShortMint.equals(longShortMint), "longShortMint stored");
    assert.ok(epoch.shortLongMint.equals(shortLongMint), "shortLongMint stored");
    assert.ok(epoch.shortShortMint.equals(shortShortMint), "shortShortMint stored");
    assert.equal(epoch.longLongSupply.toNumber(), 0);
    assert.equal(epoch.longShortSupply.toNumber(), 0);
    assert.equal(epoch.shortLongSupply.toNumber(), 0);
    assert.equal(epoch.shortShortSupply.toNumber(), 0);
  });

  it("30. split_claim: LONG token splits into LONG_LONG + LONG_SHORT", async () => {
    const NODE_ID = new BN(100);
    const SPLIT_AMOUNT = new BN(1_000_000); // 1 LONG token (6 dec)

    const callerLongAta = ata(longMint, admin.publicKey);
    const callerLongLongAta = ata(longLongMint, admin.publicKey);
    const callerLongShortAta = ata(longShortMint, admin.publicKey);
    const feeTreasuryLongLongAta = ata(longLongMint, feeTreasury);

    const [claimNodePda] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("claim_node"),
        epochPda.toBuffer(),
        admin.publicKey.toBuffer(),
        NODE_ID.toArrayLike(Buffer, "le", 8),
      ],
      program.programId
    );

    const epochBefore = await program.account.epoch.fetch(epochPda);
    const longLongSupplyBefore = epochBefore.longLongSupply.toNumber();

    await setOraclePrice(oracleKp, PRICE_100);

    await program.methods
      .splitClaim(EPOCH_ID, NODE_ID, SPLIT_AMOUNT)
      .accounts({
        epoch: epochPda,
        claimNode: claimNodePda,
        sourceMint: longMint,
        leftChildMint: longLongMint,
        rightChildMint: longShortMint,
        callerSourceAta: callerLongAta,
        callerLeftAta: callerLongLongAta,
        callerRightAta: callerLongShortAta,
        feeTreasuryAta: feeTreasuryLongLongAta,
        feeTreasury,
        config: protocolConfig,
        oracle: oracleKp.publicKey,
        caller: admin.publicKey,
    ...
    const cn = await program.account.claimNode.fetch(claimNodePda);
    assert.equal(cn.nodeId.toNumber(), NODE_ID.toNumber());
    assert.ok(cn.epoch.equals(epochPda));
    assert.ok(cn.owner.equals(admin.publicKey));
    assert.equal(cn.depth, 1);
    assert.equal(cn.isActive, true);
    assert.ok(cn.leftChildMint.equals(longLongMint));
    assert.ok(cn.rightChildMint.equals(longShortMint));
    assert.ok(cn.splitPrice.toNumber() > 0);

    const epochAfter = await program.account.epoch.fetch(epochPda);
    assert.ok(
      epochAfter.longLongSupply.toNumber() >= longLongSupplyBefore + SPLIT_AMOUNT.toNumber(),
      "longLongSupply should increase by amount"
    );
    assert.ok(epochAfter.longShortSupply.toNumber() > 0, "longShortSupply should increase");

    const llBal = await getAccount(connection, callerLongLongAta);
    const lsBal = await getAccount(connection, callerLongShortAta);
    assert.ok(Number(llBal.amount) > 0, "Caller should have LONG_LONG tokens");
    assert.ok(Number(lsBal.amount) > 0, "Caller should have LONG_SHORT tokens");
  });

  it("31. merge_claims: LONG_LONG + LONG_SHORT merge back to LONG", async () => {
    const NODE_ID = new BN(100);
    ...
    const llAccount = await getAccount(connection, callerLongLongAta);
    const lsAccount = await getAccount(connection, callerLongShortAta);
    const mergeAmount = new BN(Math.min(Number(llAccount.amount), Number(lsAccount.amount)));
    assert.ok(mergeAmount.toNumber() > 0, "Need tokens to merge");

    const longAtaBefore = await getAccount(connection, callerLongAta);
    const longBalBefore = Number(longAtaBefore.amount);

    await program.methods
      .mergeClaims(EPOCH_ID, NODE_ID, mergeAmount)
      .accounts({
        epoch: epochPda,
        claimNode: claimNodePda,
        leftChildMint: longLongMint,
    ...
    const longAtaAfter = await getAccount(connection, callerLongAta);
    assert.equal(
      Number(longAtaAfter.amount),
      longBalBefore + mergeAmount.toNumber(),
      "Caller should receive mergeAmount of LONG tokens back"
    );
  });

  it("32. split_claim fails with InvalidClaimDepth if source is a depth-2 mint", async () => {
    const NODE_ID_BAD = new BN(999);
    const BAD_AMOUNT = new BN(1_000_000);

    const callerLongLongAta = ata(longLongMint, admin.publicKey);
    const callerLongShortAta = ata(longShortMint, admin.publicKey);
    const feeTreasuryAta = ata(longLongMint, feeTreasury);
    ...
    const [claimNodePda] = PublicKey.findProgramAddressSync(
      [
        ...
        epochPda.toBuffer(),
        admin.publicKey.toBuffer(),
        NODE_ID_BAD.toArrayLike(Buffer, "le", 8),
      ],
      program.programId
    );
    ...
    try {
      await program.methods
        .splitClaim(EPOCH_ID, NODE_ID_BAD, BAD_AMOUNT)
        .accounts({
          epoch: epochPda,
          claimNode: claimNodePda,
          sourceMint: longLongMint, // depth-2 → invalid as split source
          leftChildMint: longLongMint,
          rightChildMint: longShortMint,
          callerSourceAta: callerLongLongAta,
          callerLeftAta: callerLongLongAta,
          callerRightAta: callerLongShortAta,
        ...
        .signers([admin])
        .rpc();
      assert.fail("Expected InvalidClaimDepth error");
    } catch (err: any) {
      assert.include(err.message, "InvalidClaimDepth");
    }
  });

  it("33. merge_claims fails for non-existent claim node", async () => {
    const DEAD_NODE_ID = new BN(777);
    const DEAD_AMOUNT = new BN(1);

    const callerLongLongAta = ata(longLongMint, admin.publicKey);
    const callerLongShortAta = ata(longShortMint, admin.publicKey);
    const callerLongAta = ata(longMint, admin.publicKey);

    const [deadNodePda] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("claim_node"),
        epochPda.toBuffer(),
        admin.publicKey.toBuffer(),
        DEAD_NODE_ID.toArrayLike(Buffer, "le", 8),
      ],
      program.programId
    );
    ...
    try {
      await program.methods
        .mergeClaims(EPOCH_ID, DEAD_NODE_ID, DEAD_AMOUNT)
        .accounts({
          epoch: epochPda,
          claimNode: deadNodePda,
          leftChildMint: longLongMint,
          rightChildMint: longShortMint,
          parentMint: longMint,
          callerLeftAta: callerLongLongAta,
        ...
        .signers([admin])
        .rpc();
      assert.fail("Expected error for non-existent claim node");
    } catch (err: any) {
      assert.ok(
        err.message.includes("ClaimNodeInactive") ||
